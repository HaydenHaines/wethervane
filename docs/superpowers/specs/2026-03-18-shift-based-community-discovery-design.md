...
**Step 1 — Adjacency graph.** Queen contiguity from tract geometries via `libpysal.weights.Queen`. Tracts sharing even a single point are neighbors. Produces a sparse connectivity matrix.
...
Produces a full dendrogram. Every cut height yields a different community map.
...
3. Elbow point = default operating cut
4. Expose a "resolution" parameter for finer/coarser exploration
